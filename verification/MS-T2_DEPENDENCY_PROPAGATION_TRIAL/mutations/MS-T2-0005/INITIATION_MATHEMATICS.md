...
- \(T\) = evidentiary interpretive support;
...
An observer satisfies the minimum initiation condition only when every required component is verified or verified with declared uncertainty.
...
when the observer can reproduce the canonical identity, definition, boundary, dependency structure, uncertainty state, and evidentiary basis of a governed object without relying on unsupported inference.
...
Let \(U_c\) represent the canonical uncertainty state and \(U_o\) the observer-preserved uncertainty state.

Uncertainty integrity requires:
...
10. Uncertainty must remain preserved.
...
An observer becomes eligible for participation only when canonical meaning, framework identity, dependency order, constitutional boundaries, interpretive support, uncertainty, and reconstruction competence are demonstrably preserved.
